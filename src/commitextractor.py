...
for commit in fullRepository.traverse_commits():
    ...
    sql = "INSERT INTO test.commit(datum, hashvalue, username, emailadress, remark, idproject) VALUES (%s, %s, %s, %s, %s, %s);"
    ...
    for file in commit.modified_files:
        ...
        if not (file.filename.endswith('.zip') or file.filename.endswith('.eot') or file.filename.endswith('.woff') or  file.filename.endswith('interface.saveScore.loadScore.txt') ):
            # sla op in database
            filecursor = connectie.cursor()

            sql = "INSERT INTO test.bestandswijziging (tekstvooraf, tekstachteraf, verschil, locatie, idcommit) VALUES (%s, %s, %s, %s, %s)"
